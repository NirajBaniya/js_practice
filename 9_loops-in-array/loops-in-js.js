...
// 1. forEach Method (call a function once for each array element)
...
const getArrayValues = (value, index)=>{
 ...
 const arr_numbers = [ 10, 40, 65, 70, 100];
 ...
 // map method creates a new array whereas foreach method perform operation on some array 
 ...
 // 1. Example
  ...
  // 2. Example (Print the out of the movie name whose release date is greater than 2000)
   ...
   const movie_details = [{
    ...
    // 5. every() [ Every method checks if all array value passes the test]   like a AND gate if all value is true it return true otherwise false...

    const numbers_data = [22, 25, 33, 23, 77];
    const numbers_to_check = numbers_data.every((value)=> {
        return value >= 22
    })
    console.log(numbers_to_check);

    // some() [Some method checks if one of the array value passes the test]  like a OR gate if one value is ture then it return ture...
    ...
    const results_name_details = name_details.indexOf("Blackberry",1);
    console.log(results_name_details);

    // 7. LastIndexOf() [ lastIndexOf method searches the value from last position in array in return its position]
    ...
    const find_numbers = [12, 34, 45, 23, 77]
